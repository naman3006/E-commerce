...
export class CoBrowsingGateway implements OnGatewayConnection, OnGatewayDisconnect {
    @WebSocketServer()
    server: Server;
    ...
    private users = new Map<string, { socketId: string; sessionId: string; username: string; color: string; role: 'HOST' | 'GUEST' }>();
        ...
        if (userId) {
            ...
            if (user) {
                ...
                if (session) {
                    ...
                    this.server.to(sessionId).emit('participant_left', { userId, socketId: client.id });
    ...
    handleCreateSession(@ConnectedSocket() client: Socket, @MessageBody() data: { username: string; userId?: string }) {
    ...
    handleJoinSession(@ConnectedSocket() client: Socket, @MessageBody() data: { sessionId: string; username: string; userId?: string }) {
    ...
    handleRejoinSession(@ConnectedSocket() client: Socket, @MessageBody() data: { sessionId: string; userId: string; username: string }) {
    ...
    handleNavigate(@ConnectedSocket() client: Socket, @MessageBody() data: { path: string }) {
        const userId = this.socketToUser.get(client.id);
        const user = this.users.get(userId);
        if (user) {
    ...
    handleSyncAction(@ConnectedSocket() client: Socket, @MessageBody() data: { type: string; payload: any }) {
        const userId = this.socketToUser.get(client.id);
        const user = this.users.get(userId);
        if (user) {
    ...
    handleReaction(@ConnectedSocket() client: Socket, @MessageBody() data: { emoji: string }) {
        const userId = this.socketToUser.get(client.id);
        const user = this.users.get(userId);
        if (user) {
