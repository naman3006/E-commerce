...
export class CouponsService {
    ...
    async create(createCouponDto: CreateCouponDto, userId: string): Promise<CouponDocument> {
        ...
        // Validate dates
        if (new Date(createCouponDto.validFrom) >= new Date(createCouponDto.validUntil)) {
        ...
        const coupon = new this.couponModel({
            ...
            applicableCategories: createCouponDto.applicableCategories?.map(id => new Types.ObjectId(id)) || [],
            applicableProducts: createCouponDto.applicableProducts?.map(id => new Types.ObjectId(id)) || [],
            applicableUsers: createCouponDto.applicableUsers?.map(id => new Types.ObjectId(id)) || [],
        });
        ...
        try {
            ...
            const discountText = createCouponDto.discountType === 'percentage'
            ...
            for (const user of allUsers) {
                await this.notificationsService.create(user._id.toString(), message, 'promotion');
            }
        } catch (error) {
    ...
    async update(id: string, updateCouponDto: UpdateCouponDto): Promise<CouponDocument> {
        ...
        if (updateCouponDto.validFrom && updateCouponDto.validUntil) {
            if (new Date(updateCouponDto.validFrom) >= new Date(updateCouponDto.validUntil)) {
                throw new BadRequestException('Valid from date must be before valid until date');
            }
        }
        ...
        if (updateCouponDto.applicableCategories) {
            updateData.applicableCategories = updateCouponDto.applicableCategories.map(id => new Types.ObjectId(id));
        }

        if (updateCouponDto.applicableProducts) {
            updateData.applicableProducts = updateCouponDto.applicableProducts.map(id => new Types.ObjectId(id));
        }

        if (updateCouponDto.applicableUsers) {
            updateData.applicableUsers = updateCouponDto.applicableUsers.map(id => new Types.ObjectId(id));
        }
    ...
    async validateCoupon(
        ...
        if (coupon.minPurchaseAmount > 0 && validateDto.cartTotal < coupon.minPurchaseAmount) {
            return {
                valid: false,
        ...
        if (coupon.applicableCategories.length > 0 && validateDto.categoryIds) {
            const hasApplicableCategory = validateDto.categoryIds.some(catId =>
                coupon.applicableCategories.some(appCat => appCat.toString() === catId)
            );
        ...
        if (coupon.applicableProducts.length > 0 && validateDto.productIds) {
            const hasApplicableProduct = validateDto.productIds.some(prodId =>
                coupon.applicableProducts.some(appProd => appProd.toString() === prodId)
            );
